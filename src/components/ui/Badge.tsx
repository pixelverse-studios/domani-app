...
export const Badge = ({ children, variant = 'default', className }: BadgeProps) => (
  <View className={clsx('flex-row items-center rounded-full px-3 py-1', badgeClasses[variant], className)}>
    <Text className={clsx('text-xs font-semibold uppercase tracking-tight', textClasses[variant])}>{children}</Text>
  </View>
);
