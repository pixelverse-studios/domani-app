...
1. ❌ Analytics completely broken (no data showing)
2. ❌ User categories not appearing in analytics
3. ❌ App crashes on startup
4. ❌ Tasks cannot be created or completed
5. ❌ Bright colors still present (theme regression)
6. ❌ Data loss or corruption
7. ❌ Authentication broken

**Can release with (but document):**

1. ⚠️ Minor animation jank
2. ⚠️ Non-critical visual inconsistencies
3. ⚠️ PostHog not tracking (since it's logging only)
4. ⚠️ Edge case layout issues with extreme data
