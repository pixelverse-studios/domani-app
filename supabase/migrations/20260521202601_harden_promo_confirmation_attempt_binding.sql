-- Harden service-side promo confirmation so a RevenueCat webhook can consume
-- only a fresh validation attempt for the exact store product the campaign
-- advertised. Promo validation is eligibility only; this function binds that
-- eligibility to the confirmed store transaction.
...
'Service-only promo confirmation. Requires a fresh validation attempt and exact campaign product match after server-side purchase evidence confirms lifetime access.';
