-- Confirm a current user's validated promo attempt after client-side
-- RevenueCat sync has verified a lifetime purchase. This covers the path where
-- RevenueCat's webhook arrives before the profile row can be resolved and the
-- app subsequently syncs access directly.
...
BEGIN
  ...
  SET
    ...
    response_payload = response_payload || JSONB_BUILD_OBJECT(
      'status', 'confirmed',
      'confirmedAt', NOW(),
      'confirmationSource', 'client_access_sync'
    )
  WHERE id = v_attempt.id
    AND status = 'valid'::public.promo_redemption_status
  RETURNING *
...
'Confirms and consumes the current authenticated user''s validated promo redemption attempt after client-side RevenueCat access sync verifies the expected lifetime product.';
